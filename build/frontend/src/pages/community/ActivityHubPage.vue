...
const featuredWorkshop = computed(() => workshops.value[0] ?? null);
const previewWorkshops = computed(() => workshops.value.slice(0, 3));
const previewPosts = computed(() => posts.value.slice(0, 2));
const remainingSeats = (item?: WorkshopBrief | null) => Math.max(0, (item?.capacity ?? 0) - (item?.taken ?? 0));
  ...
  <div class="activity-page">
    ...
    <main class="activity-content">
      ...
      <button
        class="workshop-hero"
        type="button"
        @click="router.push(featuredWorkshop ? `/workshop/${featuredWorkshop.id}` : '/workshops')"
      >
        <img :src="workshopHero" alt="Workshop 推荐" />
        <div class="workshop-hero__overlay">
          <strong>{{ featuredWorkshop?.title ?? 'WORKSHOP' }}</strong>
          <span>
            {{ featuredWorkshop ? `${featuredWorkshop.startDate} · 剩余 ${remainingSeats(featuredWorkshop)} 位` : '查看最新 Workshop' }}
          </span>
        </div>
      </button>

      <section class="feed-section" aria-labelledby="workshop-title">
        <div class="feed-section__head">
          <h2 id="workshop-title">Workshop 预览</h2>
          <div class="feed-section__actions">
            <button type="button" @click="router.push('/workshops')">查看全部</button>
          </div>
        </div>

        <p v-if="loading" class="feed-empty">加载中</p>
        <p v-else-if="previewWorkshops.length === 0" class="feed-empty">暂无 Workshop</p>
        <article
          v-for="item in previewWorkshops"
          :key="item.id"
          class="feed-card"
          @click="router.push(`/workshop/${item.id}`)"
        >
          <h3>{{ item.title }}</h3>
          <p>{{ item.area }} · {{ item.styles.join('/') }} · ¥{{ item.priceMin }} · 剩 {{ remainingSeats(item) }} 位</p>
        </article>
      </section>

      <section class="feed-section" aria-labelledby="post-title">
        <div class="feed-section__head">
          <h2 id="post-title">同城动态</h2>
          <div class="feed-section__actions">
        ...
        <p v-if="loading" class="feed-empty">加载中</p>
        <p v-else-if="previewPosts.length === 0" class="feed-empty">暂无同城动态</p>
        <article
          v-for="item in previewPosts"
          :key="item.id"
          class="feed-card"
          @click="router.push(`/community/post/${item.id}`)"
        >
          <h3>@{{ item.authorName }} 发布了动态</h3>
          <p>{{ item.location ?? item.style ?? '社区' }} · {{ item.topics.map((topic) => `#${topic}`).join(' ') || '新内容' }}</p>
        </article>
      </section>
    </main>
  </div>
...
.workshop-hero {
  ...
  &__overlay {
    ...
    gap: 8px;
    background: linear-gradient(180deg, rgba(17, 17, 17, 0.05), rgba(17, 17, 17, 0.7));
    color: #fff;
    text-align: left;
